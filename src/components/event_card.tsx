        ...
        <Link href={`/event/${event.id}`}>
            <div className="rounded-2xl p-3 bg-(--background) shadow-[0px_0px_12px_3px_#808080]">
                ...
                <div className="my-3 font-bold">
                    <p>🗓️: {new Date(Date.parse(event.datetime)).toLocaleString("en-AU")}</p>
                    <p>📍: {event.location}</p>
                </div>

                <p className="mb-3">{ event.description != null ? (trimmedDescription + (trimmedDescription == event.description ? "" : "...")) : ""}</p>
